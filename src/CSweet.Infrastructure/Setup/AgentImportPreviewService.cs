...
public sealed partial class AgentImportPreviewService : IPluginImportService
{
    ...
    public static void ValidateManifest(PluginManifest manifest)
    {
        ...
        else if (manifest.Protocol.MinimumVersion is not ("2.0" or "2.1" or "2.2" or "2.3") ||
        ...
        foreach (var capability in manifest.Provides)
        {
            ...
            if (capability.ExecutionTimeoutSeconds is < 1 or > PluginCapabilityDeclaration.MaximumExecutionTimeoutSeconds)
                errors.Add(
                    $"provides capability '{capability.Name}' executionTimeoutSeconds must be between 1 and {PluginCapabilityDeclaration.MaximumExecutionTimeoutSeconds}.");
