...
builder.Services.AddHostedService<PluginConnectionCleanupWorker>();
builder.Services.AddHostedService<ConnectorActionWorker>();
builder.Services.AddScoped<ConnectorActionEventDispatcher>();
...
builder.Services.AddHostedService<BusinessCalendarWorker>();
...
if (llmQueue.MaximumConcurrentRequests < 1 || llmQueue.MaximumQueuedRequests < 1 ||
    llmQueue.GenerationTimeoutSeconds < 1 || llmQueue.MaximumRequestBytes < 0 ||
    llmQueue.MaximumMessageCount < 1 || llmQueue.MaximumMessageCharacters < 0 || llmQueue.MaximumToolCount < 1 ||
...
builder.Services.AddScoped<IPlatformCapabilityHandler, CompanyReportingCapabilityHandler>();
