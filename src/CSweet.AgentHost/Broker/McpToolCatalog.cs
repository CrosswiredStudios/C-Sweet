    ...
    IOptions<McpGatewayOptions>? gatewayOptions = null)
{
    ...
    [
        ...
        HiddenWrite(PlatformCapabilities.ConnectorActionCancel, "cancel_connector_action",
            "Cancel this installation's action before execution; uncertain outcomes cannot be cancelled away."),
        ...
        Write(W.DecisionCapabilityNames.RequestV1, "request_workstream_decision",
            "Create one durable, correlated decision request with options, evidence, deadline, and blocking impact. Pending requests appear as actionable review widgets in the requesting agent's Communications chat with the business owner. Replaying the same idempotency key presents an existing pending request without duplicating it."),
        ...
        Write(CommunicationHubCapabilities.AskUser, "ask_user",
            "Ask the user one structured multiple-choice question with two to four mutually exclusive options. Put the recommended option first. The UI adds Something else unless configurationChange proposes a preset change to the requesting employee: supply key, currentValue, and proposedValue; the platform renders Switch and Leave unchanged, saves an owner-approved switch, and delivers com.csweet.agent.configuration-choice.answered.v1 to the subscribed employee."),
        ...
        HiddenWrite(ProjectIntakeCapabilities.Retain, "project_intake_retain", "Typed project prerequisite operation; never grants project creation or membership authority."),
        HiddenRead(ProjectIntakeCapabilities.Read, "project_intake_read", "Typed project prerequisite operation; never grants project creation or membership authority."),
        HiddenRead(ProjectIntakeCapabilities.List, "project_intake_list", "Typed project prerequisite operation; never grants project creation or membership authority."),
        HiddenRead(ProjectIntakeCapabilities.Discover, "project_intake_discover", "Typed project prerequisite operation; never grants project creation or membership authority."),
        HiddenWrite(ProjectIntakeCapabilities.Choose, "project_intake_choose", "Typed project prerequisite operation; never grants project creation or membership authority."),
        HiddenWrite(ProjectIntakeCapabilities.Start, "project_intake_start", "Typed project prerequisite operation; never grants project creation or membership authority."),
        HiddenWrite(ProjectIntakeCapabilities.Manager, "project_intake_manager", "Typed project prerequisite operation; never grants project creation or membership authority."),
        HiddenRead(ProjectIntakeCapabilities.AssistanceList, "project_assistance_list", "Discover the assigned Chief of Staff intake requests for bounded recovery."),
        HiddenWrite(ProjectIntakeCapabilities.Staffing, "project_intake_staffing", "Typed project prerequisite operation; never grants project creation or membership authority."),
        HiddenRead(ProjectIntakeCapabilities.ManagerSetup, "project_intake_managersetup", "Typed project prerequisite operation; never grants project creation or membership authority."),
    ...
    private static JsonElement InputFor(string capability)
    {
        ...
        W.DecisionCapabilityNames.RequestV1 => Schema("""
            {"type":"object","required":["workstreamId","typeKey","summary","authorityRuleKey","options","recommendedOptionId","evidence","blockingImpact","idempotencyKey"],"properties":{"workstreamId":{"type":"string","format":"uuid"},"typeKey":{"type":"string","minLength":1,"maxLength":200},"summary":{"type":"string","minLength":1,"maxLength":4000},"authorityRuleKey":{"type":"string","minLength":1,"maxLength":200},"options":{"type":"array","minItems":2,"items":{"type":"object"}},"recommendedOptionId":{"type":"string"},"evidence":{"type":"array","items":{"type":"object"}},"dueAt":{"type":["string","null"],"format":"date-time"},"blockingImpact":{"type":"string","minLength":1,"maxLength":4000},"supersedesDecisionId":{"type":["string","null"],"format":"uuid"},"typeData":{"type":["object","null"]},"idempotencyKey":{"type":"string","minLength":1,"maxLength":200}},"additionalProperties":false}
            """),
        ...
        ArtifactPlatformCapabilities.PackageCreate => Schema("""
            {"type":"object","required":["name","packageType","members","idempotencyKey"],"properties":{"name":{"type":"string","minLength":1,"maxLength":256},"packageType":{"type":"string","minLength":1,"maxLength":160},"members":{"type":"array","minItems":1,"maxItems":50,"items":{"type":"object","required":["artifactId","position","requiredDocumentType"],"properties":{"artifactId":{"type":"string","format":"uuid"},"position":{"type":"integer","minimum":0},"requiredDocumentType":{"type":"string","minLength":1,"maxLength":160},"acceptedRevisionId":{"type":["string","null"],"format":"uuid"}},"additionalProperties":false}},"idempotencyKey":{"type":"string","minLength":1,"maxLength":200}},"additionalProperties":false}
            """),
        ...
        CommunicationCapabilities.ChatCreate => Schema("""
            {"type":"object","required":["isDirect","isPrivate","participantOrganizationUserIds"],"properties":{"title":{"type":["string","null"],"maxLength":256},"description":{"type":["string","null"],"maxLength":2048},"isDirect":{"type":"boolean"},"isPrivate":{"type":"boolean"},"participantOrganizationUserIds":{"type":"array","maxItems":250,"items":{"type":"string","format":"uuid"}},"audienceRoleIds":{"type":["array","null"],"items":{"type":"string","format":"uuid"}},"audienceWorkstreamIds":{"type":["array","null"],"items":{"type":"string","format":"uuid"}},"workstreamId":{"type":["string","null"],"format":"uuid"},"teamId":{"type":["string","null"],"format":"uuid"}},"additionalProperties":false}
            """),
        ...
        PlatformCapabilities.ConnectorActionRequest => Schema("""
            {"type":"object","properties":{"capability":{"type":"string","minLength":1,"maxLength":200},"input":{"type":"object"},"idempotencyKey":{"type":"string","minLength":1,"maxLength":160},"mediaSource":{"type":"object","properties":{"conversationId":{"type":"string","format":"uuid"},"messageId":{"type":"string","format":"uuid"},"attachmentId":{"type":"string","format":"uuid"}},"required":["conversationId","messageId","attachmentId"],"additionalProperties":false}},"required":["capability","input","idempotencyKey"],"additionalProperties":false}
            """),
        PlatformCapabilities.ConnectorActionRead => Schema("""
            ...
            """),
        PlatformCapabilities.ConnectorActionCancel => Schema("""
            {"type":"object","properties":{"actionId":{"type":"string","format":"uuid"},"idempotencyKey":{"type":"string","minLength":1,"maxLength":160}},"required":["actionId","idempotencyKey"],"additionalProperties":false}
            """),
        ...
        GitWorkspaceCapabilities.Publish => Schema("""
            {"type":"object","required":["workspaceId","assignmentRevision","commitMessage","proposedChangeTitle","proposedChangeBody","idempotencyKey"],"properties":{"workspaceId":{"type":"string","format":"uuid"},"assignmentRevision":{"type":"integer","minimum":1},"commitMessage":{"type":"string","minLength":1,"maxLength":512},"proposedChangeTitle":{"type":"string","minLength":1,"maxLength":256},"proposedChangeBody":{"type":"string","maxLength":32768},"idempotencyKey":{"type":"string","minLength":1,"maxLength":160},"validations":{"type":["array","null"],"maxItems":100,"items":{"type":"object","required":["command","succeeded","exitCode"],"properties":{"command":{"type":"string","minLength":1},"succeeded":{"type":"boolean"},"exitCode":{"type":"integer"},"diagnosticExcerpt":{"type":["string","null"]}},"additionalProperties":false}}},"additionalProperties":false}
            """),
