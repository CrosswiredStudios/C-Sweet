...
public sealed class ExecutionNodeGatewayService(
    CSweetDbContext db,
    ...
    ILogger<ExecutionNodeGatewayService> logger)
    : ExecutionNodeGateway.ExecutionNodeGatewayBase
{
    public override async Task Connect(
        IAsyncStreamReader<NodeControlMessage> requestStream,
        ...
        ServerCallContext context)
    {
        ...
        Guid? authenticatedNodeId = null;
        long sessionEpoch = 0;
        await foreach (var message in requestStream.ReadAllAsync(context.CancellationToken))
        {
            if (!Guid.TryParse(message.NodeId, out var nodeId) || message.ProtocolVersion != "1.0")
                throw new RpcException(new Status(StatusCode.InvalidArgument, "The node envelope is invalid."));
            if (authenticatedNodeId is null)
            {
                authenticatedNodeId = nodeId;
                sessionEpoch = message.SessionEpoch;
            }
            if (authenticatedNodeId != nodeId || sessionEpoch != message.SessionEpoch)
            ...
            if (message.BodyCase != NodeControlMessage.BodyOneofCase.Heartbeat)
            ...
            else if (message.BodyCase == NodeControlMessage.BodyOneofCase.LeaseRenewal &&
            ...
            else if (message.BodyCase == NodeControlMessage.BodyOneofCase.AssignmentStatus &&
    ...
    private void ReplaceProviderInventory(
        ExecutionNode node,
        IEnumerable<NodeProviderInventory> inventory,
        DateTimeOffset now)
    {
    ...
    private static void ValidateHeartbeat(NodeHeartbeat heartbeat)
    {
        ...
        ServerCallContext context)
    {
        if (request.ProtocolVersion != "1.0" ||
            !Guid.TryParse(request.NodeId, out var nodeId) ||
    ...
    private async Task<(Guid NodeId, Guid AssignmentId)> ValidateTunnelFrameAsync(
        ...
        CancellationToken cancellationToken)
    {
        if (!Guid.TryParse(frame.NodeId, out var nodeId) ||
    ...
    private async Task AuthorizeAsync(Guid nodeId, ServerCallContext context, CancellationToken cancellationToken)
    {
        ...
        if (options.Value.AllowInsecureDevelopmentLoopback &&
            ...
            return;
        logger.LogWarning("Rejected execution node {NodeId} because its client certificate did not match.", nodeId);
        throw new RpcException(new Status(StatusCode.Unauthenticated, "A matching node client certificate is required."));
    }

    private static Task WriteFenceAsync(
        IServerStreamWriter<ControlPlaneMessage> response,
        Guid nodeId,
        long sessionEpoch,
        Guid assignmentId,
        long epoch,
        string reason,
        CancellationToken cancellationToken) => response.WriteAsync(new ControlPlaneMessage
    {
        ProtocolVersion = "1.0",
        NodeId = nodeId.ToString("D"),
        SessionEpoch = sessionEpoch,
