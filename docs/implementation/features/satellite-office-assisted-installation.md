# C-Sweet Satellite Office Assisted Installation Plan
...
|---|---|---|
| **This machine** | The machine on which the user is viewing C-Sweet setup | Download the correct signed installer, guide the user through the required operating-system approval, then hand enrollment to the installed Satellite Office without requiring a copied command or connection code. |
...
Both paths install the independently released **C-Sweet Satellite Office** product. C-Sweet must not
restore an embedded daemon, bundle Satellite Office binaries into the headquarters deployment, or
couple Satellite Office releases to C-Sweet releases.
...
3. After installation, C-Sweet should launch a locally registered Satellite Office configurator through
...
3. C-Sweet resolves an immutable package from `satellite-office-release.json` and displays its version,
   publisher, platform, architecture, size, and signature/checksum status.
4. The user selects **Download Satellite Office**. The browser downloads the signed MSI/EXE, PKG, DEB,
   or RPM package.
...
6. The installer installs the Node and RuntimeHost services plus a small local configuration launcher.
   It registers the `csweet-satellite-office` URI scheme where the platform supports it.
...
4. The user installs Satellite Office on the other machine and enters the gateway address and code.
...
- The OS may show a confirmation before opening the Satellite Office configurator.
...
```text
csweet-satellite-office://enroll/v1#handoff=<opaque-value>
```
...
- Bound to one C-Sweet deployment, intended Satellite Office enrollment, and expected gateway origin.
...
- Redacted from UI telemetry and Satellite Office logs.
...
`CSweet.SatelliteOffice.Contracts` with the smallest dependency-light request/response contract,
increment its semantic version, pack and verify it, and update the pinned package version in both
C-Sweet and `CSweet.SatelliteOffice`. Verify both repositories with sibling project references disabled.
...
1. Resolving the current Satellite Office release manifest and compatible asset.
...
Keep `/api/satellite-offices/claim`, certificate issuance, approval, heartbeat, and gRPC control-plane
...
independent Satellite Office through the normal control plane.
...
- Show the approval card for every newly pending Satellite Office, including one whose machine name
...
## Satellite Office implementation

### Installer

Each signed package installs:

- `CSweet.SatelliteOffice.Node` service.
- `CSweet.SatelliteOffice.RuntimeHost` privileged service.
- A minimal `CSweet.SatelliteOffice.Configurator` launcher.
...
- Install under `%ProgramFiles%\CSweet\SatelliteOffice` and keep state under
  `%ProgramData%\CSweet\SatelliteOffice`.
...
- Keep binaries and state in the established Satellite Office locations and launchd identifiers.
...
| TLS certificate untrusted | Show hostname, subject, issuer, validity, and SHA-256; require explicit trust for Satellite Office only. |
...
- Add **Download Satellite Office** and accurate download/open guidance.
...
- Build the minimal Satellite Office configurator.
...
- Manifest validation rejects unsupported schemas, origins, platforms, architectures, sizes, and
  digests.
- Pending local-machine and remote-machine Satellite Offices both render approval actions.
...
- C-Sweet and Satellite Office remain independently built, tagged, and released.
