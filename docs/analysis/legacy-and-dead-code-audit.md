...
- The chat path dispatches to the agent runtime and, on failure, writes a deterministic visible fallback message instead of invoking the model provider directly (`src/CSweet.Api/Chat/ChatTurnWorker.cs`, `CompleteVisibleFailureAsync`). Failure detail is persisted on `ChatTurns.ErrorCode`/`ErrorMessage` and the turn trace; see `docs/implementation/chat-turn-diagnostics.md`. Dead remnants of the removed direct-provider fallback remain: the unused `bypassMemory`/`fallbackReason` locals and bypass-memory trace strings in `ChatTurnWorker`, the unused `DirectFallbackTimeout` option (`src/CSweet.Api/Chat/ChatTurnOptions.cs`) and its `appsettings.json` entry, and the unused `FailAsync` helper.
...
**Recommendation:** If brokered plugins are the newest intended architecture, move business-strategy planning behind a real agent/plugin capability and remove the in-process runner. The direct-provider fallback has since been removed from the chat path in favor of the deterministic visible failure; delete the dead remnants listed above. If a production resilience fallback is desired later, add it intentionally with a feature flag, explicit product semantics, and equivalent governance rather than preserving it as an alternate architecture.
